...
func TestAccLinuxVirtualMachine_authSSH(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			data.ImportStep(),
			{
				Config: testLinuxVirtualMachine_authSSHUpdated(data),
				Check: resource.ComposeTestCheckFunc(
					checkLinuxVirtualMachineExists(data.ResourceName),
				),
			},
...
func TestAccLinuxVirtualMachine_authDisablePasswordAuthUpdate(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_linux_virtual_machine", "test")
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				// Enabled
				Config: testLinuxVirtualMachine_authSSHTogglePasswordAuth(data, false),
				Check: resource.ComposeTestCheckFunc(
					checkLinuxVirtualMachineExists(data.ResourceName),
				),
			},
			...
			{
				// Disabled
				Config: testLinuxVirtualMachine_authSSHTogglePasswordAuth(data, true),
				Check: resource.ComposeTestCheckFunc(
					checkLinuxVirtualMachineExists(data.ResourceName),
				),
			},
...
func testLinuxVirtualMachine_authSSHUpdated(data acceptance.TestData) string {
	template := testLinuxVirtualMachine_template(data)
	return fmt.Sprintf(`
%s
...
func testLinuxVirtualMachine_authSSHTogglePasswordAuth(data acceptance.TestData, disablePasswordAuth bool) string {
	template := testLinuxVirtualMachine_template(data)
	return fmt.Sprintf(`
%s
...
resource "azurerm_linux_virtual_machine" "test" {
  ...
  admin_username                  = "adminuser"
  admin_password                  = "P@$$w0rd1234!"
  disable_password_authentication = %t
  network_interface_ids = [
    azurerm_network_interface.test.id,
  ]
...
`, template, data.RandomInteger, disablePasswordAuth)
}
