...
func TestAccLinuxVirtualMachine_otherCustomData(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testLinuxVirtualMachine_otherCustomData(data, "/bin/bash"),
				Check: resource.ComposeTestCheckFunc(
					checkLinuxVirtualMachineExists(data.ResourceName),
				),
			},
			...
			{
				Config: testLinuxVirtualMachine_otherCustomData(data, "/bin/zsh"),
				Check: resource.ComposeTestCheckFunc(
					checkLinuxVirtualMachineExists(data.ResourceName),
				),
			},
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					checkLinuxVirtualMachineExists(data.ResourceName),
					resource.TestCheckResourceAttr(data.ResourceName, "secret.%", "1"),
				),
			},
			data.ImportStep(),
			{
				...
				Check: resource.ComposeTestCheckFunc(
					checkLinuxVirtualMachineExists(data.ResourceName),
					resource.TestCheckResourceAttr(data.ResourceName, "secret.%", "2"),
				),
			},
			data.ImportStep(),
			{
...
func testLinuxVirtualMachine_otherCustomData(data acceptance.TestData, customData string) string {
	template := testLinuxVirtualMachine_template(data)
	return fmt.Sprintf(`
%s
